...
# Merge with flows data
flows_count_latlon<-flows_count%>%
  left_join(shp_centroid%>%select(mun,centr),by=c('mun_res'='mun'))%>%
  left_join(shp_centroid%>%select(mun,centr),by=c('mun_hosp'='mun'),suffix=c('_res','_hosp'))
flows_count_latlon%>%write.table
